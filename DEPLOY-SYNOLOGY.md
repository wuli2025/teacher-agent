...
容器对外暴露宿主端口 `8081`（容器内是 8080；默认值避开 polaris-app 的 8080，可用
`TEACHER_WEB_PORT` 改），**不要**直接抢 DSM 占用的 443/5001。统一用反代对外：
...
公网访问形如 `https://teacher.你的域名/?token=<POLARIS_AUTH_TOKEN>`。
...
水位监控：`GET http://<nas>:8081/api/status` 返回容器内存（贴近 mem_limit）、数据盘用量、
推理端点状态等，可接入告警。
...
```bash
# 命令行
docker build --build-arg POLARIS_RENDER=1 -t teacher-server:full .
# 或 compose（已接 build-arg + shm_size）
POLARIS_RENDER=1 docker compose -f docker-compose.synology.yml up -d --build
```

> ⚠ 遗留：`docker-compose.synology.yml` 从上游 fork 时没带过来，本仓库里并不存在。
> 群晖部署当前只能用根目录的 `docker-compose.yml`，或按上面的命令行方式自建镜像。
