...
/// 真运行态不在此清, 由 project_stop 负责)。
fn clear_starting_placeholder(root: &str) {
    let mut g = RUNNING.lock();
    if g.get(root)
        .map(|r| r.children.is_empty() && r.pids.is_empty())
        .unwrap_or(false)
    {
        g.remove(root);
    }
...
fn root_is_trusted(root: &Path) -> bool {
    ...
    if let Some(u) = directories::UserDirs::new() {
        bases.push(u.home_dir().join("PolarisTeacher").join("data").join("artifacts"));
    }
    bases.iter().any(|b| {
...
fn run_pipeline(app: &AppHandle, root: &str, root_path: &Path, m: Manifest) {
    ...
    for cmd in &m.setup {
        ...
        if !run_blocking(app, root, root_path, cmd) {
            clear_starting_placeholder(root);
    ...
    // 注册运行态 (供 stop / status): 占位还在(没被 project_stop 摘走)才转正。
    // 占位已消失 = 用户在装依赖/启动窗口期间点了停止 → 立即收掉刚起的服务, 不再报就绪。
    let cancelled = {
        let mut g = RUNNING.lock();
