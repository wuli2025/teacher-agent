...
import deck_shici, deck_wenyan, deck_xiaoshuo

DECKS = [
    ("语文·高考古诗词鉴赏", deck_shici),
    ("语文·高考文言文阅读突破", deck_wenyan),
    ("语文·现代文阅读·小说", deck_xiaoshuo),
]
...
for title, mod in DECKS:
    ...
    try:
        ...
        out = os.path.join(DESKTOP, f"{title}-新版.pptx")
        deck.save(out)
        print(f"[deck] (占用) saved {n} slides -> {out}", flush=True)
