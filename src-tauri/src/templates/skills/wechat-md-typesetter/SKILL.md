...
- **正文开头不要写「摘要 / 导语 / 前言 / 内容提要」段落或灰底框**——直接从第一个小标题或正文开始。
- 排版要舒展、有「配图感」：给每个 `<h2>` 前加一个贴切 emoji 当小图标；关键数据/金句/结论用 `<blockquote>` 做成卡片；段落间用 `<hr>` 适度分隔。模型不能生成真实照片，靠 emoji + 卡片 + 分隔线营造视觉点，别让长图变字墙。
...
5. **长图链路 / 截图上传**（用户选了「截图上传」或嫌弃编辑器改字数/清样式时的首选路线——
   渲染权在自己手里，编辑器只当图床，零清洗零字数问题）：
   ```bash
   # 上半场：成品 HTML → 一整张完整长图（默认 --no-slice，不切片；底色铺满无白边）
   python ~/Polaris/skills/wechat-md-typesetter/scripts/wechat_yiban.py \
     --mode snapshot --body-file "<正文.html>" --theme 米纸 --title "<标题>" --no-slice
   # 输出: 长图切片-主题/ 下的成品 .html + 长图 .png + manifest.json,先报给用户眼检
   # （极端长文想分段时去掉 --no-slice，会在段落空隙切成多张；默认尽量截一整张。）
   # 下半场：切片按序粘贴进草稿(图片是编辑器原生欢迎的操作)
   python ~/Polaris/skills/wechat-md-typesetter/scripts/wechat_yiban.py \
     --mode publish-image --slices-dir "<切片目录>" --title "<标题>" \
     --intro "<一两句真文字导语,利于摘要/搜一搜>"
   ```
   流程：先填标题+保存一次（草稿条目立刻进草稿箱，防中途断掉全盘丢失）→ 逐张贴图（落位即贴
   下一张）→ 统一等素材换成 mmbiz 外链 → 再保存。失败的张会提示用户手动拖入。
   **跑 publish-image 必须给长超时（建议 ≥600s）**：含扫码登录等待，绝不能用默认 2 分钟
