...
把教师助手（Tauri 桌面 AI 教学工作台）跑成**浏览器访问的容器服务**。
...
# 3) 浏览器打开（宿主端口默认 8081，避开 polaris-app 的 8080；容器内仍是 8080）
#   http://localhost:8081
#   若设了口令：http://localhost:8081/?token=<你的口令>
```

健康检查：`curl http://localhost:8081/api/health` → `ok`。
...
- Docker 构建：`cargo build --bin teacher-server --no-default-features --features server` —— 不拉 Tauri，Linux 无需 webkit2gtk。
...
| 卷 | 容器内路径 | 内容 |
|---|---|---|
| `teacher-data` | `/root/Polaris` | 知识库 `PolarisKB/`、对话历史、项目、产物、技能 |
| `teacher-claude` | `/root/.claude` | claude 凭证、`settings.json`（供应商切换/OAuth 登录态） |
| `teacher-gitea-data` | `/data`（gitea 容器） | git 仓库存储，仅 `--profile gitea` 时用到 |
...
- **OAuth 订阅（Claude Pro / Codex）**：无头容器难走设备码流程。变通：把已登录的
  `~/.claude` 内容拷进 `teacher-claude` 卷复用。本期主推 API Key。
- **访问口令**：设 `POLARIS_AUTH_TOKEN` 后，`/api/*` 需 `Authorization: Bearer <口令>`，
  WS 需 `?token=<口令>`。前端用 `http://host:8081/?token=<口令>` 访问会自动记住口令。
...
docker exec -it teacher-web bash    # 进容器排查（claude --version 等）
```
